...
struct LoRaPacketPacked { 
    uint8_t buffer[8]; 
    LoRaPacketUnpacked unpack() {
        LoRaPacketUnpacked output;

        // First byte is all the flags packed bit-by-bit.
        output.flag_debug    = (buffer[0] & 0b00000001) != 0;
        output.flag_launch   = (buffer[0] & 0b00000010) != 0;
        output.flag_burnout  = (buffer[0] & 0b00000100) != 0;
        output.flag_apogee   = (buffer[0] & 0b00001000) != 0;
        ...
        memcpy(&packed_gps_lat, &buffer[1], 2);
        memcpy(&packed_gps_long, &buffer[3], 2);
        ...
        uint16_t packed_alt;
        memcpy(&packed_alt, &buffer[5], 2);
        output.altitude = (float)packed_alt;
        // Following byte is 4-bit SIV and 4-bit battery voltage.
        output.siv = (buffer[7] & 0b11110000) >> 4;
        output.battery_voltage = (float)(buffer[7] & 0b00001111) * 10.0 + 3.2;
...
struct LoRaPacketUnpacked {
    ...
    LoRaPacketPacked pack() {
        ...
        // where each increment of "1" represents 1/10^5 degrees, giving a total range of 15/10^5 = 0.32768 degrees or 36km in each direction,
        // along with a resolution of 1/10^5 = 0.00001 degrees or 1.11m.
        uint16_t packed_gps_lat = (uint16_t)((latitude - GPS_OFFSET_LAT) * 100000.0);
        uint16_t packed_gps_long = (uint16_t)((longitude - GPS_OFFSET_LONG) * 100000.0);
        // Following two bytes are altitude, truncated to 1 meter resolution and fit into 16 bytes.
        // This gives a maximum range of 65536 m.
        //
        ...
        memcpy(&output.buffer[7], &packed_siv_volt);
        return output;
    }
};